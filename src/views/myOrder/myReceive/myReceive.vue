      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <div style="text-align:left;">
            <!--<el-button v-if="scope.row.status === 'WAIT'" type="text" @click="cancelOrder(scope.row.id)">取消订单</el-button>-->
            <!--<el-button v-if="scope.row.status !== 'SIGNED'" type="text" @click="urgeOrder(scope.row.id)">催件</el-button>-->
            <el-button class="btn-round" v-if="scope.row.mailNo" type="text" @click="complainOrder(scope.row)">投诉</el-button>
            <el-button class="btn-round" v-if="scope.row.status === 'SIGNED'" type="text" @click="evalOrder(scope.row)">评价</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    evalOrder(row) {
      ...
      this.$router.push({ path: '/evaluate', query: { orderId: data.id, status: data.status, mailNo: data.mailNo, type: 'receive' } });
    },
    // 详情
    detail(id) {
      this.$emit('detail', id);
